...
import { MapPin, TrendingUp, Clock, Mountain, Star, Droplet, Flame, Backpack, AlertTriangle, Calendar, Thermometer, Wind, Cloud, CloudRain, Sun, ArrowLeft } from "lucide-react";
...
export default function TrailDetails({ trail: rawTrail, onBack }) {
  ...
  const mockTrail = {
    ...
    description: "A strenuous hike with spectacular views and narrow ridges. Features chains for safety on exposed sections.",
  ...
  const trail = {
    ...(rawTrail || mockTrail),
    difficulty_rating: (rawTrail?.difficulty_rating || mockTrail.difficulty_rating),
    elevation_gain: (rawTrail?.elevation_gain || rawTrail?.elevation || mockTrail.elevation_gain),
    elevation: (rawTrail?.elevation_gain || rawTrail?.elevation || mockTrail.elevation),
    length: parseFloat(rawTrail?.length || mockTrail.length),
    altitude_ft: (rawTrail?.altitude_ft || rawTrail?.elevation_gain || mockTrail.altitude_ft),
    visitor_usage: (rawTrail?.visitor_usage || mockTrail.visitor_usage),
    state_name: (rawTrail?.state_name || rawTrail?.location?.split(',')[1]?.trim() || mockTrail.state_name),
    area_name: (rawTrail?.area_name || rawTrail?.location?.split(',')[0]?.trim() || mockTrail.area_name),
  ...
  const parseTrailCharacteristics = (trailData) => {
    ...
    const hasPaved = features.includes('paved') || features.includes('partially-paved');
    const hasForest = features.includes('forest');
    const hasRiver = features.includes('river') || features.includes('waterfall');
    ...
    const elevGain = parseFloat(trailData.elevation_gain) || parseFloat(trailData.elevation) || 0;
    ...
    const bearStates = ['Alaska', 'Montana', 'Wyoming', 'Idaho', 'Colorado', 'Washington', 
                        'California', 'Oregon', 'Arizona', 'New Mexico', 'Utah', 'Vermont', 
                        'New Hampshire', 'Maine', 'Minnesota', 'Wisconsin', 'Michigan', 
  ...
  const estimateHikeTime = (distance, elevationGain, difficulty, experience, packWeight = 10) => {
  ...
  const calculateWaterAmount = () => {
    ...
    if (!timeHours || timeHours <= 0) return {
      totalLiters: 2,
      totalOunces: 67.6,
      estimatedHours: 2,
      ratePerHour: 1,
    ...
    const safetyMargin = (weatherInputs.temperature_f > 90 || trailChar.distanceMiles > 10 || altFt > 10000) ? 1.0 : 0.5;
    ...
    return {
      ...
      ratePerHour: Math.round((Math.max(waterLiters, 1) / timeHours) * 100) / 100,
  ...
  const calculateCalories = () => {
    ...
    const ageFactor = userInputs.age > 45 ? 1 - 0.004 * (userInputs.age - 45) : 1 + 0.004 * (45 - userInputs.age);
    ...
    let caloriesBurned = (distanceCalories + elevationCalories) * ageFactor * heightFactor;
  ...
  const generateGearRecommendations = () => {
    ...
    return {
      packSize,
      gearByCategory: gear,
      totalItems: Object.values(gear).reduce((sum, items) => sum + items.length, 0)
    };
  };
  ...
  const generateSafetyNotes = () => {
    ...
    if (trailChar.difficultyRating >= 6) {
      notes.push("⚠️ VERY STRENUOUS: Only attempt if in excellent physical condition");
    }
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-green-50 to-blue-50">
        ...
        <div className="absolute bottom-0 left-0 right-0 p-8 text-white">
          <div className="max-w-6xl mx-auto">
            <div className="flex items-center gap-3 mb-2">
              <span className={`px-4 py-2 rounded-full text-sm font-semibold ${difficultyColors[trail.difficulty]}`}>
                {trail.difficulty}
              </span>
              <div className="flex items-center gap-1">
                <Star className="w-5 h-5 fill-yellow-400 text-yellow-400" />
                <span className="font-semibold">{trail.rating}</span>
                <span className="text-gray-300 text-sm">({trail.reviews} reviews)</span>
              </div>
            </div>
      ...
      <div className="max-w-6xl px-4 py-8 ml-60">
        ...
        <div className="bg-white rounded-xl shadow-md p-6 mb-8">
          <div className="grid grid-cols-4 gap-6 text-center">
            <div>
              ...
              <div className="text-2xl font-bold text-gray-900">{parseFloat(trail.length).toFixed(1)} mi</div>
            </div>
            <div>
              <TrendingUp className="w-6 h-6 text-green-600 mx-auto mb-2" />
              <div className="text-sm text-gray-600">Elevation Gain</div>
              <div className="text-2xl font-bold text-gray-900">{(parseFloat(trail.elevation_gain) || 0).toLocaleString()} ft</div>
            </div>
            <div>
              <Clock className="w-6 h-6 text-green-600 mx-auto mb-2" />
              <div className="text-sm text-gray-600">Duration</div>
              <div className="text-2xl font-bold text-gray-900">{trail.duration}</div>
            </div>
            <div>
              <Mountain className="w-6 h-6 text-green-600 mx-auto mb-2" />
              <div className="text-sm text-gray-600">Altitude</div>
              <div className="text-2xl font-bold text-gray-900">{(parseFloat(trail.altitude_ft) || 0).toLocaleString()} ft</div>
            </div>
          </div>
        </div>
          ...
          <div className="flex items-start justify-between">
            <div className="flex-1">
              ...
              <div className="flex items-center gap-4 mb-4">
                ...
                {weatherInputs.forecast === 'rain' && <CloudRain className="w-16 h-16" />}
                {weatherInputs.forecast === 'snow' && <Cloud className="w-16 h-16" />}
                <div>
                  <p className="text-5xl">{weatherInputs.temperature_f}°</p>
                  <p className="text-blue-100 capitalize">{weatherInputs.forecast === 'clear' ? 'Mostly sunny' : weatherInputs.forecast}</p>
                </div>
              </div>
              <div className="grid grid-cols-3 gap-4 text-sm">
                <div className="flex items-center gap-2">
                  <Droplet className="w-4 h-4" />
                  <span>{weatherInputs.forecast === 'rain' ? '70%' : '10%'} rain</span>
                </div>
                <div className="flex items-center gap-2">
        ...
        {/* Input Forms */}
        <div className="bg-white rounded-xl shadow-md p-6 mb-8">
          <h2 className="text-2xl font-bold text-gray-900 mb-6">Get Personalized Recommendations</h2>
              ...
              <div className="space-y-4">
                <div>
                  ...
                  <input
                    ...
                    onChange={(e) => setUserInputs({...userInputs, weight_lbs: parseInt(e.target.value)})}
                ...
                <div>
                  ...
                  <input
                    ...
                    onChange={(e) => setUserInputs({...userInputs, age: parseInt(e.target.value)})}
                    className="w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-green-500"
                  />
                </div>
                <div>
                  ...
                  <input
                    ...
                    onChange={(e) => setUserInputs({...userInputs, height_inches: parseInt(e.target.value)})}
                    className="w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-green-500"
                  />
                </div>
                <div>
                  <label className="block text-sm font-medium text-gray-700 mb-1">Pack Weight (lbs)</label>
                  <input
                    type="number"
                    value={userInputs.pack_weight_lbs}
                    onChange={(e) => setUserInputs({...userInputs, pack_weight_lbs: parseInt(e.target.value)})}
                    className="w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-green-500"
                  />
                </div>
                <div>
              ...
              <div className="space-y-4">
                <div>
                  ...
                  <input
                    ...
                    onChange={(e) => setWeatherInputs({...weatherInputs, temperature_f: parseInt(e.target.value)})}
                    className="w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-green-500"
                  />
                </div>
                <div>
                  ...
                  <input
                    ...
                    onChange={(e) => setWeatherInputs({...weatherInputs, humidity_percent: parseInt(e.target.value)})}
                    className="w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-green-500"
                  />
                </div>
                <div>
                  ...
                  <input
                    ...
                    onChange={(e) => setWeatherInputs({...weatherInputs, wind_mph: parseInt(e.target.value)})}
                    className="w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-green-500"
                  />
                </div>
                <div>
                  ...
                  <select
                    ...
                    onChange={(e) => setWeatherInputs({...weatherInputs, forecast: e.target.value})}
                    className="w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-green-500"
                  >
                ...
                <div>
                  ...
                  <select
                    ...
                    onChange={(e) => setWeatherInputs({...weatherInputs, season: e.target.value})}
                    className="w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-green-500"
                  >
          ...
          <>
            ...
            <div className="bg-red-50 border-l-4 border-red-500 rounded-lg p-6 mb-8">
              <div className="flex items-start">
                ...
                <div className="flex-1">
                  <h3 className="text-lg font-bold text-red-900 mb-3">Safety Notes</h3>
                  <ul className="space-y-2">
                    {recommendations.safety.map((note, idx) => (
                      <li key={idx} className="text-red-800">{note}</li>
                    ))}
                  </ul>
                </div>
              </div>
              ...
              <div className="bg-white rounded-xl shadow-md p-6">
                ...
                <div className="space-y-3">
                  <div>
                    ...
                    <div className="text-2xl font-bold text-gray-900">{recommendations.water.totalLiters}L</div>
                    <div className="text-sm text-gray-500">({recommendations.water.totalOunces} oz)</div>
                  </div>
                  <div>
                    <div className="text-sm text-gray-600">Bottles Needed</div>
                    <div className="text-lg font-semibold text-gray-900">{recommendations.water.bottlesNeeded} × 24oz bottles</div>
                  </div>
                  <div>
                    <div className="text-sm text-gray-600">Consumption Rate</div>
                    <div className="text-lg font-semibold text-gray-900">{recommendations.water.ratePerHour}L/hour</div>
                  </div>
                </div>
              </div>
              ...
              <div className="bg-white rounded-xl shadow-md p-6">
                ...
                <div className="space-y-3">
                  <div>
                    ...
                    <div className="text-2xl font-bold text-gray-900">{recommendations.calories.caloriesBurned}</div>
                  </div>
                  <div>
                    <div className="text-sm text-gray-600">Food to Carry</div>
                    <div className="text-lg font-semibold text-gray-900">{recommendations.calories.foodWeightLbs} lbs</div>
                    <div className="text-sm text-gray-500">({recommendations.calories.foodCaloriesNeeded} calories)</div>
                  </div>
                  <div>
              ...
              <div className="bg-white rounded-xl shadow-md p-6">
                ...
                <div className="space-y-3">
                  <div>
                    ...
                    <div className="text-2xl font-bold text-gray-900">{recommendations.water.estimatedHours} hours</div>
                  </div>
                  <div>
                    <div className="text-sm text-gray-600">Recommendation</div>
                    <div className="text-lg font-semibold text-gray-900">Start early (6-8am)</div>
                  </div>
                  <div className="pt-3 border-t border-gray-200">
                    <p className="text-xs text-gray-600">💡 Add 30% buffer time. Turn back if behind schedule by midday</p>
                  </div>
                </div>
              </div>
            </div>
            ...
            <div className="bg-white rounded-xl shadow-md p-6">
              <div className="flex items-center mb-6">
                ...
                <div>
                  <h3 className="text-2xl font-bold text-gray-900">Gear Recommendations</h3>
                  <p className="text-sm text-gray-600">Recommended pack: {recommendations.gear.packSize}</p>
                </div>
              </div>
              ...
              <div className="grid md:grid-cols-2 gap-6">
                {/* Ten Essentials */}
                <div>
                  <h4 className="font-semibold text-gray-900 mb-3 text-lg">Ten Essentials</h4>
                  <ul className="space-y-2">
                    {recommendations.gear.gearByCategory.tenEssentials.slice(0, 8).map((item, idx) => (
                ...
                <div className="space-y-4">
                  {recommendations.gear.gearByCategory.footwear.length > 0 && (
                    <div>
                      <h4 className="font-semibold text-gray-900 mb-2">Footwear</h4>
                      <ul className="space-y-1">
                        {recommendations.gear.gearByCategory.footwear.map((item, idx) => (
                          <li key={idx} className="text-sm text-gray-700 flex items-start">
                            <span className="text-green-600 mr-2">✓</span>
                            {item}
                          </li>
                  ...
                  {recommendations.gear.gearByCategory.terrainSpecific.length > 0 && (
                    <div>
                      <h4 className="font-semibold text-gray-900 mb-2">Terrain Specific</h4>
                      <ul className="space-y-1">
                        {recommendations.gear.gearByCategory.terrainSpecific.map((item, idx) => (
                          <li key={idx} className="text-sm text-gray-700 flex items-start">
                            <span className="text-green-600 mr-2">✓</span>
                            {item}
                          </li>
                  ...
                  {recommendations.gear.gearByCategory.environmental.length > 0 && (
                    <div>
                      <h4 className="font-semibold text-gray-900 mb-2">Environmental</h4>
                      <ul className="space-y-1">
                        {recommendations.gear.gearByCategory.environmental.map((item, idx) => (
                          <li key={idx} className="text-sm text-gray-700 flex items-start">
                            <span className="text-green-600 mr-2">✓</span>
                            {item}
                          </li>
                  ...
                  {recommendations.gear.gearByCategory.safety.length > 0 && (
                    <div>
                      <h4 className="font-semibold text-gray-900 mb-2">Safety & Navigation</h4>
                      <ul className="space-y-1">
                        {recommendations.gear.gearByCategory.safety.map((item, idx) => (
                          <li key={idx} className="text-sm text-gray-700 flex items-start">
