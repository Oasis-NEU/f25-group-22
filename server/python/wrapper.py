...
def get_altitude_estimate(state):
    """
    Estimate average altitude for a state in feet.
    """
    state_altitudes = {
        'Colorado': 6800,
        'Wyoming': 6700,
    ...
    return state_altitudes.get(state, 1000)

def estimate_hike_time(distance_miles, elevation_gain_ft, difficulty, experience, pack_weight_lbs=15):
    """
    Quick time estimate helper function.
    """
    base_time = (distance_miles / 2) + (elevation_gain_ft / 2000)
...
def generate_hiking_recommendations(df_hike, df_user, df_weather):
    ...
    recommendations = {
        ...
        'gear': {
            'pack_size': gear.get('pack_size_recommended', 'Not specified'),
            'estimated_pack_weight_lbs': gear.get('estimated_pack_weight_lbs', 0),
            'gear_by_category': gear.get('gear_by_category', {}),
            'total_items': gear.get('total_gear_items', 0)
        },
...
if __name__ == '__main__':
    ...
    df_hike = pd.DataFrame([input_data['hike']])
    df_user = pd.DataFrame([input_data['user']])
    df_weather = pd.DataFrame([input_data['weather']])
    
    result = generate_hiking_recommendations(df_hike, df_user, df_weather)
